...
BEGIN IMPORT:
```{r}
#Import Race/Ethnicity for Boise MSA
acs_race <- getCensus(
  key = CENSUS_KEY,  
  name = "acs/acs1", 
  vintage = 2019, 
  vars = c("group(B03002)"), 
  region = "metropolitan statistical area/micropolitan statistical area:14260")
```

```{r}
#Import Educational Attainment for Boise MSA
acs_edu <- getCensus(
  key = CENSUS_KEY,  
  name = "acs/acs1", 
  vintage = 2019, 
...
```{r}
boise_census_df <- list(age_df, race_df, edu_df)
```

```{r}
saveRDS(boise_census_df, 'boise_census_df.rds')
```
`
