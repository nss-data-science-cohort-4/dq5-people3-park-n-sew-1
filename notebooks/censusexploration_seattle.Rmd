...
BEGIN IMPORT:
```{r}
#Import Race/Ethnicity for Seattle MSA
acs_race <- getCensus(
  key = CENSUS_KEY,  
  name = "acs/acs1", 
  vintage = 2019, 
  vars = c("group(B03002)"), 
  region = "metropolitan statistical area/micropolitan statistical area:42660")
```

```{r}
#Import Educational Attainment for Seattle MSA
acs_edu <- getCensus(
  key = CENSUS_KEY,  
  name = "acs/acs1", 
  vintage = 2019, 
...
```{r}
seattle_census_df <- list(age_df, race_df, edu_df)
```

```{r}
saveRDS(seattle_census_df, 'seattle_census_df.rds')
```
`
