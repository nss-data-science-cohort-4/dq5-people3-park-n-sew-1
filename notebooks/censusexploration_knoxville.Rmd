...
BEGIN IMPORT:
```{r}
#Import Race/Ethnicity for Knoxville MSA
acs_race <- getCensus(
  key = CENSUS_KEY,  
  name = "acs/acs1", 
  vintage = 2019, 
  vars = c("group(B03002)"), 
  region = "metropolitan statistical area/micropolitan statistical area:28940")
```

```{r}
#Import Educational Attainment for Knoxville MSA
acs_edu <- getCensus(
  key = CENSUS_KEY,  
  name = "acs/acs1", 
  vintage = 2019, 
...
```{r}
knoxville_census_df <- list(age_df, race_df, edu_df)
```

```{r}
saveRDS(knoxville_census_df, 'knoxville_census_df.rds')
```
`
