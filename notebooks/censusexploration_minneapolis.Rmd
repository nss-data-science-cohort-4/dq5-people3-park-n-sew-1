...
BEGIN IMPORT:
```{r}
#Import Race/Ethnicity for Minneapolis MSA
acs_race <- getCensus(
  key = CENSUS_KEY,  
  name = "acs/acs1", 
  vintage = 2019, 
  vars = c("group(B03002)"), 
  region = "metropolitan statistical area/micropolitan statistical area:33460")
```

```{r}
#Import Educational Attainment for Minneapolis MSA
acs_edu <- getCensus(
  key = CENSUS_KEY,  
  name = "acs/acs1", 
  vintage = 2019, 
...
```{r}
#Import Age for Minneapolis MSA
acs_age <- getCensus(
  key = CENSUS_KEY,  
  name = "acs/acs1", 
  vintage = 2019, 
...
```{r}
minneapolis_census_df <- list(age_df, race_df, edu_df)
```

```{r}
saveRDS(minneapolis_census_df, 'minneapolis_census_df.rds')
```
`
