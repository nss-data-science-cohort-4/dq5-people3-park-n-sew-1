...
BEGIN IMPORT:
```{r}
#Import Race/Ethnicity for Chattanooga MSA
acs_race <- getCensus(
  key = CENSUS_KEY,  
  name = "acs/acs1", 
  vintage = 2019, 
  vars = c("group(B03002)"), 
  region = "metropolitan statistical area/micropolitan statistical area:16860")
```

```{r}
#Import Educational Attainment for Chattanooga MSA
acs_edu <- getCensus(
  key = CENSUS_KEY,  
  name = "acs/acs1", 
  vintage = 2019, 
...
```{r}
#Import Age for Chattanooga MSA
acs_age <- getCensus(
  key = CENSUS_KEY,  
  name = "acs/acs1", 
  vintage = 2019, 
...
```{r}
chattanooga_census_df <- list(age_df, race_df, edu_df)
```

```{r}
saveRDS(chattanooga_census_df, 'chattanooga_census_df.rds')
```
`
